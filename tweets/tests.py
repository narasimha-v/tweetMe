...
class TweetTestCase(TestCase):
    def setUp(self):
        ...
        Tweet.objects.create(content='tweet#3', user=self.user_a)
        self.currentCount = Tweet.objects.all().count()

    def test_tweet_created(self):
        tweet = Tweet.objects.create(content='tweet#4', user=self.user)
        self.assertEqual(tweet.id, 4)
        self.assertEqual(tweet.user, self.user)

    def get_client(self):
        client = APIClient()
        client.login(username=self.user.username, password='random!23')
        return client
